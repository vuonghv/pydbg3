...
import os
from ctypes import windll, CDLL, c_char_p, byref
...
class PDError(Exception):
    ...
    and includes support for automated Windows error message
    resolution and formatting.

    For example, to raise a generic error you can use::

        raise PDError('Badness occurred.')

    To raise a Windows API error you can use::

        raise PDError('SomeWindowsApi()', True)
    """

    def __init__(self, message: str = '', win32_exception: bool = False):
        """

        @param message:
        @param win32_exception:
        """
    ...
    def __str__(self):
        ...
        return '[{:d}] {!s}: {!s}'.format(
            self.error_code, self.message, self.error_msg.value
        )
